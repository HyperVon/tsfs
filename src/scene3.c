...
unsigned char MOTION1[65536],MOTION2[65536],MOTION3[65536],MOTION4[65536],motion_que[5][64000];

OBJECT object[50],*object_ptr;

unsigned char anti_aliased_buffer[64000];

int findRGB2(unsigned char R, unsigned char G, unsigned char B,unsigned char *pal)
{
...
main(int argc, char *argv[])
{
 ...
 int done,i,j,k,l,m,num_objects,p;
 unsigned char *buf1,pal[768],shade_type,*pix_ptr,rc,gc,bc;
 ...
 texturemap = (unsigned char *) malloc(12*65536*sizeof(unsigned char));
 if(texturemap == NULL)
 {
     printf("no mem avail\n");
     exit(1);
 ...
 fp = fopen("motion1.lut","rb");
 fread(MOTION1,sizeof(unsigned char),65536,fp);
 fclose(fp);

 fp = fopen("motion2.lut","rb");
 fread(MOTION2,sizeof(unsigned char),65536,fp);
 fclose(fp);

 fp = fopen("motion3.lut","rb");
 fread(MOTION3,sizeof(unsigned char),65536,fp);
 fclose(fp);

 fp = fopen("motion4.lut","rb");
 fread(MOTION4,sizeof(unsigned char),65536,fp);
 fclose(fp);
...
// Wait_For_Vsync();
 ...
 memset(motion_que[p],0,VIDEO_STORAGE);
...
while(!done)
 ...
 //copy double buffer to the screen
// Wait_For_Vsync();
 memcpy(motion_que[4],buffer,64000);

 for(i=0;i<64000;i++)
  buffer[i] = MOTION1[(motion_que[0][i]<<8)];

 for(i=0;i<64000;i++)
  buffer[i] = MOTION2[(motion_que[1][i]<<8)+buffer[i]];

 for(i=0;i<64000;i++)
  buffer[i] = MOTION3[(motion_que[2][i]<<8)+buffer[i]];

 for(i=0;i<64000;i++)
  buffer[i] = MOTION4[(motion_que[3][i]<<8)+buffer[i]];

 for(i=0;i<64000;i++)
  if(motion_que[4][i] != 0)
   buffer[i]=motion_que[4][i];

 memcpy(video_buffer,buffer,64000);

 memcpy(motion_que[0],motion_que[1],64000);
 memcpy(motion_que[1],motion_que[2],64000);
 memcpy(motion_que[2],motion_que[3],64000);
 memcpy(motion_que[3],motion_que[4],64000);
