...
// Old_Isr = _dos_getvect(DIV_BY_0);
// _dos_setvect(DIV_BY_0, div_by);
...
/*
 for(i=0;i<num_objects-1;i++)
  for(k=0;k<object[i].num_polygons;k++)
   if(k % 2 == 0)
   {
    object[i].polys[k].texturemap = texturemap+65536;
   }
   else
   {
    object[i].polys[k].texturemap = texturemap;
   }
*/
...
while(!done)
 {
  ...
  if(kbhit())
   switch(getch())
   {
    case 'a': camera.viewpoint.x-=65536/6;
              break;
    case 's': camera.viewpoint.x+=65536/6;
              break;
    case 'w': camera.viewpoint.y+=65536/6;
              break;
    case 'z': camera.viewpoint.y-=65536/6;
              break;
