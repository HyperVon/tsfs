...
put_sprite128x128(int x, int y, unsigned char *bmap, Fixed32 scale_factor)
{
 int sx,sy,c,s;
 int end_y,end_x,start_x,start_y;
 Fixed32 scale;
 ...
 scale = fixeddiv(8388608,scale_factor);

 bmap_x = bmap_y = 0;
 map_inc = fixeddiv(8388608,scale);
 scale >>= 17;
 end_y =  y + scale - 1;
 start_x = x - scale;
 start_y = y - scale;
...
main(int argc, char *argv[])
{
 ...
 sprite = texturemap+3*65536;
 world_save18.x = object[18].world_pos.x;
 world_save18.y = object[18].world_pos.y;
 world_save18.z = object[18].world_pos.z;
 ...
 set_sprite3d(&object[num_objects],
              ...
              temp2,
              0,NON_ANIMATED);
 object[num_objects].polys[0].texturemap = firemap;
 num_objects++;
...
while(!done)
 ...
 {
  object[k].x_ang+=1;
  if(object[k].x_ang>=180) object[k].x_ang = 0;

//  object[k].y_ang+=1;
//  if(object[k].y_ang>=180) object[k].y_ang = 0;

  object[k].z_ang+=1;
  if(object[k].z_ang>=90) object[k].z_ang = 0;
 }
