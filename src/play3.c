...
main(int argc, char *argv[])
{
 ...
 unsigned char pal[768],shade_type,pal_right_shift[768];
...
/* initialize graphics mode */
 ...
 pre_right_shift_pal(pal_right_shift,pal);
...
// Wait_For_Vsync();
 ...
 for(y=2;y<198;y++)
  for(x=2;x<318;x++)
  {
//   pix_ptr = buffer + (y*320) + x;
   rsum = gsum = bsum = 0;

   // o
   r = pal[buffer[y*320+x]*3];
   g = pal[buffer[y*320+x]*3+1];
   b = pal[buffer[y*320+x]*3+2];
   ...
   // n
   r = pal[buffer[(y-1)*320+x]*3];
   g = pal[buffer[(y-1)*320+x]*3+1];
   b = pal[buffer[(y-1)*320+x]*3+2];
   ...
   // e
   r = pal[buffer[y*320+x+1]*3];
   g = pal[buffer[y*320+x+1]*3+1];
   b = pal[buffer[y*320+x+1]*3+2];
   ...
   rc = rsum; gc = gsum; bc = bsum;
   if(rc == 0 && gc == 0 && bc == 0)
    rc = bc = gc = 1;
//   printf("%d %d %d\n",rc,gc,bc);

   anti_aliased_buffer[(y*320)+x] =
     findRGB(rc,gc,bc,pal);
 }
*/

// anti_alias(anti_aliased_buffer,buffer,pal_right_shift);
// memcpy(video_buffer,anti_aliased_buffer,64000);
