...
unsigned char secondary_buffer[64000];
...
main(int argc, char *argv[])
{
 ...
 FILE *path,*world,*fp;
 PATH flight_path,light_path,target_path,secondary_flight_path,secondary_target_path;
 POINT_3D *save_flight_path_ptr,*save_target_path_ptr;
 float r,g,b,rsum,gsum,bsum;
 int x,y,cross_fade_count,save_num_target_path_points,save_num_flight_path_points;
 unsigned char *cross_fade_table[9],*save_buffer_ptr;
 ...
 fclose(path);
 generate_spline_path(&secondary_flight_path,runtime*maxfps);
 generate_spline_path(&secondary_target_path,runtime*maxfps);
...
*/
 ...
 for(i=0;i<9;i++)
  cross_fade_table[i] = (unsigned char *) malloc(sizeof(unsigned char)*65536);
 ...
 for(i=8;i<num_objects;i++)
 {
  object[i].z_ang = fixedmul(fixeddiv(65536,((rand()%4)<<16)),(5<<16));
  printf("%f\n",object[i].z_ang/65536.0);
 }
...
/* initialize graphics mode */
 ...
 save_flight_path_ptr = flight_path.path;
 save_target_path_ptr = target_path.path;
 save_num_flight_path_points = flight_path.num_path_points;
 save_num_target_path_points = target_path.num_path_points;
...
while(!done)
 ...
 for(k=8;k<num_objects;k++)
 {
  object[k].world_pos.x = sine[(object[k].x_ang>>16)];
  object[k].x_ang+=object[k].z_ang;
  if(object[k].x_ang>=(360<<16)) object[k].x_ang = 0;
 }
 ...
 for (k=0;k < num_objects; k++)
  ...
  if(cross_fade_table_index == 18)
  {
   ...
   if(cross_fade_count % 2 == 0)
   {
    flight_path.path = secondary_flight_path.path;
    flight_path.num_path_points = secondary_flight_path.num_path_points;
    target_path.path = secondary_target_path.path;
    target_path.num_path_points = secondary_target_path.num_path_points;
   }
   else
   {
    flight_path.path = save_flight_path_ptr;
    flight_path.num_path_points = save_num_flight_path_points;
    target_path.path = save_target_path_ptr;
    target_path.num_path_points = save_num_target_path_points;
   }
   cross_fade_count++;
  }
 }//end if cross_fade
