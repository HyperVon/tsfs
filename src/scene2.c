...
unsigned char *texturemap_copy[7];
int maxfps,runtime;
FILE *out;
...
{
 POINT_3D temp,temp2,cam_save;
 POINT_3D_float lightsource_orgi;
 VECTOR_3D A;
 int done,i,j,k,l,m,num_objects;
 unsigned char pal[768],shade_type;
 char t,text_buffer[80];
 int shade,mirror_count;
 float start,end,frames;
 OBJECT object[15],*object_ptr;
 FILE *path,*world,*fp;
 ...
 mirror_buffer = (unsigned char *) malloc(65536*sizeof(unsigned char));
 memset(mirror_buffer,0,65536);

 for(i=1;i<7;i++)
  texturemap_copy[i] = (unsigned char *) malloc(65536);
 ...
 {
  object[i-1].mirror=i;
  for(j=0;j<object[i-1].num_polygons;j++)
   object[i-1].polys[j].mirror=i;
 }
...
while(!done)
 ...
 for(k=0;k<num_objects;k++)
 {
  ...
  object[k].y_ang+=3;
  if(object[k].y_ang>=360) object[k].y_ang = 0;
 ...
 for (k=0;k < num_objects; k++)
  ...
  cam_save.x = camera.viewpoint.x;
  cam_save.y = camera.viewpoint.y;
  cam_save.z = camera.viewpoint.z;
  mirror_count=0;
 ...
 for(m=1;m<7;m++)
 {
  ...
  //vector from us to the center of the object
  A.x = cam_save.x - object[m-1].world_pos.x;
  A.y = cam_save.y - object[m-1].world_pos.y;
  A.z = cam_save.z - object[m-1].world_pos.z;

  //see if we can even see this plane if not ignore it
  if( (fixedmul(A.x,object[m-1].polys[0].normal.x) +
       fixedmul(A.y,object[m-1].polys[0].normal.y) +
       fixedmul(A.z,object[m-1].polys[0].normal.z)) > 0)
  {
  mirror_count++;
  memset(mirror_buffer,0,65536);

  camera.viewpoint.x = object[m-1].world_pos.x;
  camera.viewpoint.y = object[m-1].world_pos.y;
  camera.viewpoint.z = object[m-1].world_pos.z;

  camera.lookat.x = object[m-1].world_pos.x + object[m-1].polys[0].normal.x;
  camera.lookat.y = object[m-1].world_pos.y + object[m-1].polys[0].normal.y;
  camera.lookat.z = object[m-1].world_pos.z + object[m-1].polys[0].normal.z;
  ...
  for(i=0;i<num_polys;i++)
  {
       ...
       if(shade_type == ENVMAP)
         texture_mirror(master_facet_list[i]->quad,
          master_facet_list[i]->transparent);
       else if(shade_type == FLATTEX)
         tmapflat_mirror(master_facet_list[i]->quad,
                  master_facet_list[i]->transparent);
       else if(shade_type == GOURTEX)
         tmapgour_mirror(master_facet_list[i]->quad,
                  master_facet_list[i]->transparent);
       else if(shade_type == GOURAUD)
         gouraud_mirror(master_facet_list[i]->quad,
  ...
  memcpy(texturemap_copy[m],object[m-1].polys[0].texturemap,65536);

  for(i=0;i<65536;i++)
  if(mirror_buffer[i])
   texturemap_copy[m][i] = TRANSPAR[(mirror_buffer[i]<<8)+object[m-1].polys[0].texturemap[i]];

  }//end if that checks if we can see this object
 } 
  ...
  for(i=0;i<num_polys;i++)
  {
       ...
       if(master_facet_list[i]->mirror)
          texturemap = texturemap_copy[master_facet_list[i]->mirror];
 ...
 frames++;
 sprintf(text_buffer,"fps: %2.2f NP: %d M: %d",frames/((end-start)/100.0),num_polys,mirror_count);
