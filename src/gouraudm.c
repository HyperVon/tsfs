void gouraud_mirror(TEXTURE * vtx,unsigned char color,unsigned char transparent)
{
    TEXTURE * v1 = vtx;
    TEXTURE * v2 = vtx+1;
    TEXTURE * v3 = vtx+2;
    ...
    if( (v3->y < 0 || v1->y > 255) || (v1->x < 0 && v2->x < 0 && v3->x < 0) ||
                              (v1->x > 255 && v2->x > 255 && v3->x > 255))
      return;
...
//    printf("2 divides\n");
    ...
    destptr = (char *)((v1->y<<8) + mirror_buffer);
    ...
    for(;;)         
    {
        ...
        if(width > 0 && destptr >= mirror_buffer && destptr < (mirror_buffer+65280) && x1 < 255)
        {
...
//     return;
    ...
    for(;;)         
    {
	    ...
	    do
            {
                *dest++ = TRANSPAR[(LUT[(color<<6)+(i>>8)]<<8) + *dest];
                i += didx;
            }
            while(--width);
